...
class MainUi(QtWidgets.QMainWindow):
    ...
    def init_ui(self):
        ...
        self.left_label_1 = QtWidgets.QPushButton('WELCOME，' + C.USER[3])
        self.left_label_1.setObjectName('left_label')
        self.left_label_3 = QtWidgets.QPushButton("PERSONAL INFORMATION")
        self.left_label_3.setObjectName('left_label')

        self.left_button_1 = QtWidgets.QPushButton(qtawesome.icon('fa.users', color='white'), "MEMBER MANAGEMENT")
        self.left_button_1.setObjectName('left_button')
        self.left_button_1.clicked.connect(lambda: self.setRightWidget(C.FLAG_STUDENT))
        self.left_button_2 = QtWidgets.QPushButton(qtawesome.icon('fa.id-card', color='white'), "SCORE MANAGEMENT")
        self.left_button_2.setObjectName('left_button')
        self.left_button_2.clicked.connect(lambda: self.setRightWidget(C.FLAG_GRADE))
        # 管理员账号
        if C.USER[0] == 1:
            self.left_button_3 = QtWidgets.QPushButton(qtawesome.icon('fa.institution', color='white'), "TEAM MANAGEMENT")
            self.left_button_3.setObjectName('left_button')
            self.left_button_3.clicked.connect(lambda: self.setRightWidget(C.FLAG_CLASS))
            self.left_button_4 = QtWidgets.QPushButton(qtawesome.icon('fa.user-secret', color='white'), "PLAYER MANAGEMENT")
        ...
        self.left_button_8 = QtWidgets.QPushButton(qtawesome.icon('fa.pencil-square-o', color='white'), "CHANGE INFO")
        self.left_button_8.setObjectName('left_button')
        self.left_button_8.clicked.connect(lambda: self.setRightWidget(C.FLAG_INFO))
        self.left_button_9 = QtWidgets.QPushButton(qtawesome.icon('fa.sign-out', color='white'), "LOG OUT")
    ...
    def student_management(self):
        ...
        self.pushButton_del.setText(_translate("Form", "DELETE"))
        self.pushButton_3.setText(_translate("Form", "IMPORT"))
        ...
        item.setText(_translate("Form", "OPERATION"))
    ...
    def grade_management(self):
        ...
        if self.right_widget:
            self.main_layout.removeWidget(self.right_widget)  # 移除已有右侧组件
        self.setWindowTitle('Pythonn Sailing Management System')
        ...
        item.setText(_translate("Form", "ID"))
        item = self.tableWidget.horizontalHeaderItem(3)
        item.setText(_translate("Form", "SHOOTING"))
        item = self.tableWidget.horizontalHeaderItem(4)
        item.setText(_translate("Form", "ASSISTAING"))
        item = self.tableWidget.horizontalHeaderItem(5)
        item.setText(_translate("Form", "REBOUNDING"))
        item = self.tableWidget.horizontalHeaderItem(6)
        item.setText(_translate("Form", "STEALING"))
    ...
    def class_management(self):
        ...
        if self.right_widget:
            self.main_layout.removeWidget(self.right_widget)  # 移除已有右侧组件
        self.setWindowTitle('Python Sailing Management System')
        ...
        _translate = QtCore.QCoreApplication.translate
        self.pushButton_3.setText(_translate("Form", "ADD TEAM"))
    ...
    def admin_management(self):
        ...
        _translate = QtCore.QCoreApplication.translate
        self.pushButton_3.setText(_translate("Form", "ADD PLAYER"))
        self.label_2.setText(_translate("Form", "NAME"))
        self.pushButton_2.setText(_translate("Form", "CHECK"))
        self.pushButton_del.setText(_translate("Form", "DELETE"))
        ...
        item.setText(_translate("Form", "NAME"))
        item = self.tableWidget.horizontalHeaderItem(3)
        item.setText(_translate("Form", "ETC"))
        item = self.tableWidget.horizontalHeaderItem(4)
    ...
    def change_info(self):
        ...
        self.label_pass0.setText('old password')
        ...
        self.input_pass0.setPlaceholderText('enter old password')
        ...
        self.label_pass1.setText('new password')
        # 新密码输入框
        ...
        self.input_pass1.setPlaceholderText('inpit new password')
        ...
        self.label_pass2.setText('repeat')
        # 重复密码输入框
        ...
        self.input_pass2.setPlaceholderText('repeat')
        ...
        if button:
            ...
            if flag == C.FLAG_STUDENT:
                ...
                if class_ not in class_dict2.keys():
                    QMessageBox.critical(self, 'input error', "team not exist！")
                    return
                else:
                    try:
                        class_ = class_dict2[class_]
                        sql_execute(updateStudentById(id, name, number, sex, class_))
                        QMessageBox.about(self, 'successfullu', "claps！")
                        self.editRow(id, flag, False)
                    except Exception as e:
                        print(e)
                        QMessageBox.critical(self, 'fail', "check！")
                        return
            ...
            # 成绩管理
                ...
                if not chinese.isdigit() or not math.isdigit() or not english.isdigit():
                    QMessageBox.critical(self, 'sth run', "check ！")
                    return
                else:
                    try:
    ...
    def batchDelete(self, flag):
        ...
        for i in range(len(all_header_combobox)):
            try:
                ...
                print(e)
        if len(ids) == 1:
            QMessageBox.information(self, 'fail', "no data selected")
            return
    ...
    def deleteRow(self, ids, flag):
        ...
        if flag == C.FLAG_GRADE:
            title = 'delete all'
            tips = 'u sure?' if len(ids) == 1 else 'delete all' + str(len(ids) - 1) + 'data?'
        else:
            title = 'data delete'
            tips = 'u sure to ?' if len(ids) == 1 else 'delete' + str(len(ids) - 1) + 'data?'
            ...
            if len(ids) <= 1:
                ...
                if button:
                    ...
                    if flag == C.FLAG_STUDENT:
                        ...
                        if len(result) > 0:
                            QMessageBox.information(self, 'fail', "members in teams")
                            return
                        else:
                        ...
                        if ids[0] == 1:
                            QMessageBox.information(self, 'fail',)
                            return
                        else:
                ...
                if flag == C.FLAG_CLASS:
                    if len(errIds) == 0:
                        QMessageBox.about(self, 'ACCESS UPDATED', "DELETE TEAM %d" % (len(ids) - 1))
                    else:
                        QMessageBox.information(self, 'FINISHED',
                                                "SUCCESSFULLY delete team%d，fail%d。\nfail team id：%s\n members in teams"
    ...
    def viewTable(self, id, flag):
        ...
        if flag == C.FLAG_GRADE:
            ...
            QMessageBox.about(self, 'DERAILED', str)
    ...
    def buttonForRow(self, id, flag, info=''):
        ...
        if flag == C.FLAG_GRADE:
            ...
            deleteBtn.setText("clear all")
            # 查看
            viewBtn = QPushButton('detailed')
    ...
    def createData(self, flag):
        if flag == C.FLAG_CLASS:
            text, okPressed = QInputDialog.getText(self, "add team", "Team name:", QLineEdit.Normal, "")
            if okPressed and text != '':
                try:
                    sql_execute(insertClass(text))
                    QMessageBox.about(self, 'successful!', 'team added successfully！')
                    self.getDataList(flag, i=-1)
                except Exception as e:
                    QMessageBox.critical(self, 'fail', 'add team not successfully：\n' + e)
    ...
    def editUsername(self):
        reply = QMessageBox.question(self, 'CHANGE USERNAME', 'After changing, relogin required, are u sure？',
                                     QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
        if reply == QMessageBox.Yes:
            username = self.input_name.text()
            sql_execute(updateUsernameById(C.USER[0], username))
            QMessageBox.about(self, 'succ', 'user name changed successfully！')
            self.logOut(False)
    ...
    def modifyPassw(self):
        ...
        if pass1 != pass2:
            QMessageBox.information(self, 'FAIL', 'DIFFERENT PASSWORD ENTERED 。')
            return
        else:
            if C.USER[2] != md5(pass0):
                QMessageBox.information(self, 'FAIL', 'INCORRECT PWSWORD！')
                return
            else:
                sql_execute(resetAdminPasswById(C.USER[0], pass1))
                QMessageBox.about(self, 'SUCCESSFUL', 'PSWWORD MODIFIED SUCCESSFULLY！PLEASE RELOGIN。')
                self.logOut(False)
    ...
    def open_file(self, flag):
        ...
        data_format = ''
        if flag == C.FLAG_STUDENT:
            data_format = 'NAME/ID/GENDER/TEAM'
        elif flag == C.FLAG_GRADE:
            data_format = 'NAME/ID/SCORE/TIME/...'
        QMessageBox.information(self, 'INPUT',
                                "PLEASE SELECT CSV\n DATAFORMAT：%s，READ FROM SECOND LINE" % data_format)
        fileName, fileType = QtWidgets.QFileDialog.getOpenFileName(self, "SELECT DOCUMENT", os.getcwd()+'/data', "CSV Files(*.csv)")
        if fileName == '':
            return
        try:
        ...
        for d in data.itertuples():
            try:
                ...
                if flag == C.FLAG_STUDENT:
                    ...
                    if class_ not in class_dict2.keys():
                        fail_count += 1
                        errText += '\n %s : TEAM[%s]NOT EXISI/NO RIGHT TO VISIT！' % (d[1], class_)
                        continue
                    else:
                        class_ = class_dict2[class_]
        ...
        if fail_count > 0:
            QMessageBox.about(self, 'INPUT SUCCESS',
                              "TATA INPUT %d，FAIL%d。\nERROE INFOR AS：%s" % (success_count, fail_count, errText))
        else:
            QMessageBox.about(self, 'INPUT SUCCESS', "SUCCESSFULLY INPUT DATA %d！" % success_count)
        # 刷新列表
    ...
    def data_export(self, flag):
        ...
        if flag == C.FLAG_STUDENT:
            filename = 'Students'
            columns = ['NAME', 'ID', 'GENDER', 'TEAM']
        elif flag == C.FLAG_GRADE:
            filename = 'Grade'
            columns = ['NAME', 'ID', 'TIME', '...', '...', 'OVERALL', 'RANKING']
        # 保存文件对话框
        file_path = QFileDialog.getSaveFileName(self, "DATA INPUT", "data/%s.csv" % filename,
        ...
        try:
            ...
            QMessageBox.about(self, 'SUCCESS', "DATA OUTPUT TO ：%s" % file_path[0])
        except Exception as e:
            QMessageBox.critical(self, 'FAIL', "CANT SAVE：%s" % e)
            print(e)
